...
exports.getAll = async (req, res) => {
	...
	if (query) {
    const filteredAllItems = await knex("warehouses")
      ...
      .whereILike("warehouses.warehouse_name",`%${query}%`)
			.orWhereILike("inventories.item_name", `%${query}%`)
			.orWhereILike("inventories.description", `%${query}%`)
			.orWhereILike("inventories.category", `%${query}%`)
			.orWhereILike("inventories.status", `%${query}%`);

		res.status(200).json(filteredAllItems);
	} else {
